...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # 实验证明只有启动服务时初始化一次 Scheduler
        # print(f"[Scheduler] 初始化 Scheduler, 策略: {vllm_config.scheduler_config.policy}")
        ...
        # 策略列表：ssjf、ua、ltr、gmmua
        if self.policy in [SchedulingPolicy.SSJF, SchedulingPolicy.UA, SchedulingPolicy.LTR, SchedulingPolicy.GMM_UA]:
            try:
                ...
                print(f"[Scheduler] {self.policy.value.upper()} tokenizer 已加载: {type(tokenizer)}")
            except Exception as e:
                print(f"[Scheduler] 警告: 无法获取tokenizer: {e}")
                tokenizer = None
        else:
            print(f"警告: 策略{self.policy}不在优化策略列表中, 不加载tokenizer")
        if self.policy == SchedulingPolicy.SSJF:
            print("[SSJF] 正在加载 SSJF 预测模型...")
            ssjf_gpu = int(os.environ.get('SSJF_PREDICTOR_GPU', '-1'))
            from vllm.v1.core.sched.ssjf_predictor import (
                load_model as load_ssjf_predictor_model,
            )
            load_ssjf_predictor_model(device_id=ssjf_gpu)
            
        if self.policy == SchedulingPolicy.UA:
            print("[UA] 正在加载 UA 预测模型...")
            ua_gpu = int(os.environ.get('UA_PREDICTOR_GPU', '0'))
            from vllm.v1.core.sched.ua_predictor import (
                load_model as load_ua_predictor_model,
            )
            load_ua_predictor_model(device_id=ua_gpu)
            
        if self.policy == SchedulingPolicy.LTR:
            print("[LTR] 正在加载 LTR 预测模型...")
            ltr_gpu = int(os.environ.get('LTR_PREDICTOR_GPU', '-1'))
            from vllm.v1.core.sched.ltr_predictor import (
                load_model as load_ltr_predictor_model,
            )
            load_ltr_predictor_model(device_id=ltr_gpu)
        
        if self.policy == SchedulingPolicy.GMM_UA:
            print("[GMM_UA] 正在加载 GMM_UA 预测模型...")
            gmm_ua_gpu = int(os.environ.get('UA_PREDICTOR_GPU', '0'))
            from vllm.v1.core.sched.gmm_ua_predictor import (
                load_model as load_gmm_ua_predictor_model,
            )
            load_gmm_ua_predictor_model(device_id=gmm_ua_gpu)
            
        # 等待队列：新到达的请求首先进入这里
        # 传入policy参数以决定队列排序类型
        self.waiting = create_request_queue(self.policy, tokenizer=tokenizer)
        # 运行队列：正在被处理的请求。其实是个list
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        # 每当GPU执行一次推理阶段，都调用一次该函数(core.py中)，以确定下次推理阶段要处理哪些请求的哪些token
        """
        优先级分层: RUNNING请求 > WAITING请求
        同一队列内基本遵循FCFS, 但允许跳过暂时无法调度的请求
        抢占机制: 当KV Cache不足时, 抢占队列末尾的请求(FCFS)或优先级最低且到达最晚的请求(PRIORITY), 被抢占的请求进度清零, 回到WAITING队列头部
        """
        scheduled_new_reqs: list[Request] = []          # 决定去运行的新请求
        scheduled_resumed_reqs: list[Request] = []      # 恢复运行的请求（曾经被抢占）
        scheduled_running_reqs: list[Request] = []      # 正在运行中的请求
        preempted_reqs: list[Request] = []              # 决定抢占的请求

        req_to_new_blocks: dict[str, KVCacheBlocks] = {}    # 请求ID -> 分配的KV缓存块
        num_scheduled_tokens: dict[str, int] = {}           # 请求ID -> 本轮调度的token数
        token_budget = self.max_num_scheduled_tokens        # 本轮可调度的token预算
        ...
        # 正在运行的请求优先于等待的请求，遵循FCFS

        req_index = 0   # 正在进行调度决策的请求索引
        # 当running队列请求未遍历完，且token预算未耗尽时
        while req_index < len(self.running) and token_budget > 0:
            request = self.running[req_index] # 获取当前请求
            # 计算该请求需要调度的token数
            num_new_tokens = (
                request.num_tokens_with_spec        # 需要处理的总token数
                + request.num_output_placeholders   # 输出的占位符token数
                - request.num_computed_tokens       # 已计算的token数
            )

            # 高并发下会出现num_new_tokens < 0的bug，做简单修复
            if num_new_tokens < 0:
                import logging
                logger = logging.getLogger(__name__)
                logger.warning(
                    f"⚠️ Negative tokens for {request.request_id}: {num_new_tokens}, "
                    f"setting to 0 and skipping this round."
                )
                logger.warning(
                    f"request.prompt_token_ids: {request.prompt_token_ids}"
                )
                num_new_tokens = 0
            ...
            # 处理Encoder输入（用于encoder-decoder模型）
            ...
            with record_function_or_nullcontext("schedule: allocate_slots"):
                while True:
                    ...
                    if self.policy == SchedulingPolicy.PRIORITY:
                        preempted_req = max(
                            self.running,
                            key=lambda r: (r.priority, r.arrival_time), # 抢占优先级最低、到达最晚的
                        )
            ...
            # 调度成功
            scheduled_running_reqs.append(request)  # 加入scheduled_running_reqs列表
        ...
        # 临时队列会被频繁创建，并且迟早会回到waiting队列中，因此改为使用FCFS策略即可
        # skipped_waiting_requests = create_request_queue(self.policy)
        skipped_waiting_requests = create_request_queue(SchedulingPolicy.FCFS)

        # Next, schedule the WAITING requests.
        # 第二阶段：调度WAITING请求
        if not preempted_reqs:  # 只有没有抢占发生时才调度waiting请求
        ...
        scheduler_output = SchedulerOutput(
            scheduled_new_reqs=new_reqs_data,   # 本轮新调度的请求
            scheduled_cached_reqs=cached_reqs_data, # 本轮继续的请求
            num_scheduled_tokens=num_scheduled_tokens,  # 每个请求本轮处理的token数
            total_num_scheduled_tokens=total_num_scheduled_tokens,  # 总token数
            ...
            finished_req_ids=self.finished_req_ids, # 已完成的请求id
    ...
    def shutdown(self) -> None:
        ...
        # 关闭调度策略的后台线程（SSJF 和 UA）
        if (
            self.policy in [SchedulingPolicy.SSJF, SchedulingPolicy.UA, SchedulingPolicy.LTR]
            and hasattr(self.waiting, 'shutdown')
        ):
            self.waiting.shutdown()
            print(f"[Scheduler] {self.policy.value.upper()} 队列已关闭")
